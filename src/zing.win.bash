...
# for network host/port on Linux bash for Windows.
#
...
hostaddr=$(echo "$hosttext" | cut -f4 -d ' ' | tr '()' ' ')
hostname=$(echo "$hosttext" | cut -f3 -d ' ')
...
do
  ...
  do
    ...
    do
      ...
      result=`nc -v -z -w $var_time $var_host $port 2>&1 | cut -f1-3 -d' '`
      ...
      echo -n "op $i.$j. "
      fmt_result=`echo -n "$result" | cut -f1-3 -d' '`
