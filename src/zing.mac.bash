...
# for network host/port on MacOS.
#
...
# function computes standard deviation
stddev()
{
  val=$(echo ${time_array[*]} | 
  awk 'NF { sum=0;
...
hostaddr=$(echo "$hosttext" | cut -f2 -d ' ' | tr '[]' ' ')
hosttext=$(echo "$hosttext" | cut -f1 -d' ')
hostname=$(nc -v -z -w 2 $hostaddr 80 2>&1 | cut -f1 -d' ')
...
do
  ...
  echo -n "ZING:$hostaddr/ $hosttext / $hostname on $port is: $var_status"
  ...
  # for count number of ops

  for (( i=1 ; i<=$var_count ; i++ ));
  do

    # for limit--number of cmds for op

    for (( j=1 ; j<=$var_limit ; j++ ));
    do
      result=""

      echo -n "Port: $port: "
      start_time=`gdate +%s%N`  # gdate is for MacOS else 'date' on POSIX

      result=`nc -v -z -G $var_time $var_host $port 2>&1 | cut -f1-3 -d' ' | tr -s '\n' ' '` 

      close_time=`gdate +%s%N`
      ...
      echo -n "op $i.$j. "
      fmt_result=`echo -n "$result" | cut -f1-2 -d' '`
...
echo "--- host name: $hosttext / $hostname "
